...
public class ImportCountriesCommandHandler(IRepository<Country> repository)
    : IRequestHandler<ImportCountriesCommand, ImportResult>
{
    public async Task<ImportResult> Handle(ImportCountriesCommand request, CancellationToken cancellationToken)
    {
        ...
        foreach (var row in request.Rows)
        {
            ...
            if (string.IsNullOrWhiteSpace(row.Abbreviation))
                ...
                rowErrors.Add(new ImportError(row.LineNumber, "Abbreviation", $"A country with abbreviation '{row.Abbreviation}' already exists."));
            ...
            staged.Add(new Country { Name = row.Name!, Abbreviation = row.Abbreviation! });
        }

        if (request.Mode == ImportMode.Atomic && errors.Count > 0)
            return new ImportResult(0, errors.Count, errors);

        foreach (var country in staged)
